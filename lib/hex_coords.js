...
export const CUBE_DIRECTIONS = [
	Cube(1, -1, 0),  // 0 - bottom right
	Cube(0, -1, +1), // 1 - bottom left
	Cube(-1, 0, +1), // 2 - left
	Cube(-1, +1, 0), // 3 - top left
...
export function sharedAxialsDirection(axialA, axialB) {
	return HEX_RANGE.find((i) => areAxialsEqual(axialNeighborAtDirection(axialA, i), axialB))
}
...
export function verticesSharingAxials(axialA, axialB) {
	...
	if (direction) {
		return [Vertex(axialA, direction), Vertex(axialA, direction + 1)]
	} else {
		return []
	}
}
