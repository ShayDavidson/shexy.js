...
export function areVerticesEquivalent(vertexA, vertexB) {
	!!equivalentVertices(vertexA).find((equivalentVertex) => areVerticesEqual(equivalentVertex, vertexB))
}

export function equivalentVertices(vertex) {
	cubesSharingDiagonal(vertex.cube, vertex.i).map((cube, index) => Vertex(cube, vertex.i + (index + 1) * 2))
}
...
export const VERTEX_DIRECTIONS = [
	Vertex(Cube(0, 0, 0), -1),  // 0 -
	Vertex(Cube(+1, -1, 0), 0), // 1 -
	Vertex(Cube(0, 0, 0), +1)  // 2 -
]
