import { roundAxial, Axial, sharedAxialsDirection, HEX_RANGE } from './hex_coords'
...
export function axialsToPathCorners(axialA, axialB, size, padding = 0) {
	...
	return [
		hexCorner(axialACenter, size, direction - 1),
		hexCorner(axialACenter, size, direction),
		hexCorner(axialBCenter, size, direction + 2),
		hexCorner(axialBCenter, size, direction + 3)
	]
}
