...
export const AXIAL_DIRECTIONS = [
	Axial(+1, 0),  // 0 - | right            4 /\ 5
	Axial(0, +1),  // 1 - / bottom right    3 |  | 0
	Axial(-1, +1), // 2 - \ bottom left      2 \/ 1
	Axial(-1, 0),  // 3 - | left
	Axial(0, -1),  // 4 - / top left
	Axial(+1, -1)  // 5 - \ top right
]

// a direction spans between (diagonal-1)..(diagonal)
export const AXIAL_DIAGONALS = [
	Axial(+1, +1), // 0 - ↘ bottom right       4
	Axial(-1, +2), // 1 - ↓ bottom          3 /↑\ 5
	Axial(-2, +1), // 2 - ↙ bottom left      |↖ ↗|
	Axial(-1, -1), // 3 - ↖ top left         |↙ ↘|
	Axial(+1, -2), // 4 - ↑ top             2 \↓/ 0
	Axial(+2, -1)  // 5 - ↗ top right          1
]
...
export function areAxialsNeighbors(axialA, axialB) {
	return sharedAxialsDirection(axialA, axialB) !== undefined
}
